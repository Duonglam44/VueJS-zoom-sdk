    ...
    setup(){
      ...
      .then(response=> {
        ...
        this.device.on('connect', conn => {
            ...
            // 発話ストリーム取得
            // オペレーター
            this.twilio_connection.mediaStream.getUserMedia({ video: false, audio: true }).then(function (stream) {
                this.stream1 = stream
                // createMedia(this.stream1)
            })
            // 客
            this.stream2 = new MediaStream([this.twilio_connection.mediaStream._remoteStream.getAudioTracks()[0]])
